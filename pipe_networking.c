...
int server_setup() {
    ...
    remove("wkp"); //removes the WKP once a connection has been made
...
int server_connect(int from_client) {
        ...
        //run server stuff
        char buffer[HANDSHAKE_BUFFER_SIZE];
        read(from_client, buffer, sizeof(buffer)); //reads client's private FIFO name
        printf("[server] handshake: received [%s]\n", buffer);

        int downstream = open(buffer, O_WRONLY); //connects to client's private FIFO
        write(downstream, ACK, sizeof(ACK)); //sends initial message after connecting
        read(from_client, buffer, sizeof(buffer)); //waits for response from client
	printf("[server] handshake received: %s\n", buffer);
